...
//write protecc?
	if (substr_count(wr_db(), $_POST['write']) != 0) {
		...
		if (uidlsk($_POST['uid'], $_POST['ukey']) == true && wr_bycb($_GET['write'], 2)[0] != 'g') {
			//only login
		...
		if (uidlsk($_POST['uid'], $_POST['ukey']) == true && wr_bycb($_POST['write'], 2)[0] == 'g') {
			$thing = explode(":", wr_bycb($_POST['write'], 2));
			...
			while ($count < count($allowed)) {
				...
				$s = uid($_POST['uid'], $_POST['ukey'], 4);
				echo("checking: $allowed[$count] in array object $s");
				echo '<br><br>' . in_array($allowed[$count], explode('//', uid($_POST['uid'], $_POST['ukey'], 4)) , TRUE);
				if (in_array($allowed[$count], explode('//', uid($_POST['uid'], $_POST['ukey'], 4)) , TRUE)) {
					$yes = True;
				}
				$count = $count + 1;
			}
