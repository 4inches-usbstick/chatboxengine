...
async def on_message(message):
    if message.content.startswith('$send'):
        ...
        stx = sendmsg('71.255.240.10:8080', x[1], x[2], '', 'UTF-8')
    ...
    if message.content.startswith('$inv'):
        ...
        print(x[1])
        joinurl = 'http://'+ip+'/textengine/sitechats/inchat.php?chatnum='+x[1]+'&explorer=0&namer=&encoderm=UTF-8&refreshrate=5000'
        tosend = 'Join Chatbox **'+x[1]+'** with join URL: **'+joinurl+'**'
    ...
    if message.content.startswith('$cmd'):
        ...
        if x[1] == '$open':
            await message.channel.send("open: creates a new Chatbox. Syntax: **$open;<new chatbox number>;<option, only valid value is l>;<allowmed option, default is forbidmed>**.")
    ...
    if message.content.startswith('$close'):
        ...
        if xz[1] in tokens:
            ...
            send_command(ip, 'del', chatboxtodelete, 'merrychristmasmotherfuckers')
