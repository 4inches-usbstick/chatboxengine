...
while True:
    ...
    if commandi == "edit":
        ...
        print("Sending regular terminal commands: just enter the command, the parameter, then the key. For edit, enter the edit command, the path, the findstr, the replacewithstr, and the key (all arguments necessary). For the send command, type in the send command, the message, the path, the name, and the encoder. (message and path arguments necessary). You can make a regular GET request with exe-py. Type in the command, then the URL to GET. change-ip to change the IP of the server you want to connect to. This needs to be an IP that is CBE configured. Enter the command, then the IP (with port number). ")
    elif commandi == "exit":
        exit()
