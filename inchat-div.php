...
if ($gethttd === false) {
	...
	$mediaoptions = "<a href='$pcl://$ip/textengine/$sc/media/uploadform.php?chatnum1=$_GET[chatnum]&rr=$_GET[refreshrate]'>Upload Media, </a>
<a href=\"$pcl://$ip/textengine/$sc/inchat-div.php?chatnum=$_GET[chatnum]&refreshrate=$_GET[refreshrate]&explorer=1&encoderm=$coder&bbg=$_GET[bbg]&namer=$_GET[namer]&uid=$_GET[uid]&ukey=$_GET[ukey]\">Find / Post Media, </a>
<a href=\"$pcl://$ip/textengine/$sc/inchat-div.php?chatnum=$_GET[chatnum]&refreshrate=$_GET[refreshrate]&explorer=0&encoderm=$coder&bbg=$_GET[bbg]&namer=$_GET[namer]&uid=$_GET[uid]&ukey=$_GET[ukey]\">Close Media Finder, </a><br>";
	$type = 'leg';
}
...
if (plsk(39) == 'CHECK') {
$gethttd = file_get_contents("$pcl://$ip/textengine/$sc/media/$nub-med");
}
if (plsk(39) == 'YES') {
$gethttd = true;
}
...
if ($gethttd === false) {
	...
	$mediaoptions = "<a href='$pcl://$ip/textengine/$sc/media/uploadform4html.php?chatnum1=$_GET[chatnum]&rr=$_GET[refreshrate]'>Upload Media, </a>
<a href=\"$pcl://$ip/textengine/$sc/inchat-div.php?chatnum=$_GET[chatnum]&refreshrate=$_GET[refreshrate]&explorer=1&encoderm=$coder&bbg=$_GET[bbg]&namer=$_GET[namer]&namer=$_GET[namer]&uid=$_GET[uid]&ukey=$_GET[ukey]\">Find / Post Media, </a>
<a href=\"$pcl://$ip/textengine/$sc/inchat-div.php?chatnum=$_GET[chatnum]&refreshrate=$_GET[refreshrate]&explorer=0&encoderm=$coder&bbg=$_GET[bbg]&namer=$_GET[namer]&namer=$_GET[namer]&uid=$_GET[uid]&ukey=$_GET[ukey]\">Close Media Finder, </a><br>";
	$type = 'htm';
}
echo($type);
...
$getter = "$pcl://$ip/textengine/$sc/display.php?chatbox=$_GET[chatnum]&encode=$_GET[encoderm]&uid=$_GET[uid]&ukey=$_GET[ukey]";
...
if ($_GET['explorer'] == 1 and $type == 'leg') {
	echo("<iframe id=\"iframe2\" src=\"$pcl://$ip/textengine/$sc/media/$_GET[chatnum]/uploaded\" width=\"610\" height=\"400\">no</iframe>");
}
if ($_GET['explorer'] == 1 and $type == 'htm') {
	echo("<iframe id=\"iframe3\" src=\"$pcl://$ip/textengine/$sc/media/$nub-med/uploaded\" width=\"610\" height=\"400\">no</iframe>");
}
...
<form id=\"main0\"action=\"$form\" method=\"$way\" autocomplete=\"off\" onsubmit=\"sendmymessage(); setTimeout( function(){reloadiframe()}, 500); return false\">
<fieldset>
...
<input type=\"button\" style=\"color:black\" onclick=\"location.href='$pcl://$ip/textengine/$sc/inchat-div.php?chatnum=$_GET[chatnum]&refreshrate=foobar&encoderm=$_GET[encoderm]&namer=$_GET[namer]';\" value=\"No Polling Mode\" />
<!--input type=\"button\" style=\"color:black\" onclick=\"location.href='$pcl://$ip/textengine/$sc/high-security/media/uploadform.php';\" value=\"Upload Files\" /-->
...
//echo("<code>Join by URL:<br></code><code>$pcl://$ip/textengine/$sc/inchat_joinpage.php?chatnum=$_GET[chatnum]&refreshrate=$_GET[refreshrate]&explorer=0&encoderm=$coder&bbg=$_GET[bbg]</code>  <br>");
$useleg = plsk(19);
if ($useleg == 'YES') {
echo("
